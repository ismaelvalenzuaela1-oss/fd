...
void main() {
	...
	if(Position.z == 0.0 // z location is always 0 
		...
		pos += vec3(0.0,-8.0,0.0); // apply an offset
        /* Reposition End */
        
        /** Recolor (Remove up to Recolor End if unnecessary)**/
        //if(length(Color.rgb - vec3(0.0, 0.0, 0.0)) < 0.002) {
        //    vertexColor = vec4(0.3137, 0.0392, 0.0784, 1); // shadow (usually black)
        //} else {
        //    vertexColor = vec4(0.9058, 0.1137, 0.2117, 1); // text (usually green)
        //}
        /* Recolor End */
